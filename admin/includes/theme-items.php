...
?>
      <form class="form-edit lists theme-lists clearfix" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
        <header id="page-header" class="clearfix">
          <h1><?php echo $title; ?></h1>    
          <div class="actions">
            <a class="btn btn-blue install-theme enabled" href="javascript:void(0);">Install New Theme</a>
          </div>
          <!-- .actions -->
        </header>
        <!-- .page-header -->
...
<?php 
                  ...
                  foreach ($style as $line => $value) :
                    if (strpos($value, 'Theme Name:') !== false) :
                      ...
                      $data['theme_name'] = trim($val);
                    endif;
                      
                    if (strpos($value, 'Version:') !== false) :
                      list($key, $val) = explode(':', $value, 2);
                      $data['version'] = trim($val);
                    endif;
                    
                    if (strpos($value, 'Author:') !== false) :
                      list($key, $val) = explode(':', $value, 2);
                      $data['author']  = trim($val);
                    endif;
                    
                    if (strpos($value, 'Author URI:') !== false) :
                      list($key, $val) = explode(':', $value, 2);
                      $data['author_uri'] = trim($val);
                    endif;
                  endforeach;
                ?>
              <div class="thumbnail">
...
<?php else : ?>
                <img src="<?php admin_url(); ?>/images/no-theme-thumb.png" alt="" />
<?php endif; ?>
              </div>            
              <div class="details clearfix">
<?php if ($data['theme_name']) : ?>
                <div class="theme-name clearfix">
                  <span class="name"><?php echo trim($data['theme_name']); ?></span>
<?php if ($data['version']) : ?>
                  <span class="ver"><?php echo $data['version'];?></span>
<?php endif;
      if (!is_theme_activated($data['dir'])) : ?>
...
<?php endif; ?>
                <div class="author">
                  By <?php if (isset($data['author'])) :
                     if ($data['author_uri']) echo '<a href="'. $data['author_uri'] .'">'. $data['author'] .'</a>'; else echo $data['author'];
                    else :
                    echo 'Anonymous';
                  endif; ?></div>
              </div>
...
<?php endif;
    // if npt completed themes / themes corrupted
    if ($not_completed_themes) : ?>
